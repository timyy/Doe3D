...
function initIdentify() {
    ...
    dojo.connect(identifyTask, "onComplete", "identifyCallback");//注意上下文
    dojo.connect(identifyTask, "onError", "identifyErrorback");//注意上下文
}
...
function create3D(center, results) {
    dojo.forEach(results, function (item, index, results) {
        ...
        if (!bExist) {
            if (item.geometryType == "esriGeometryPolyline" && item.layerName.indexOf('管线') >= 0) {
            ...
            else if (item.geometryType == "esriGeometryPoint" && item.layerName.indexOf('管点') >= 0) {
...
function onRotateSceneMouseMove(currentX, currentY) {
    var RotateRadianY = 0.0;//围绕Y轴旋转的角度
    ...
    if (scene.pipeOperationMode == "ROTATE")//只绕Y轴旋转
    {
...
function GetColorByPipeType(PipeType) {
    var PipeColor;
    var type = PipeType;
    if (PipeType.indexOf("管线") >= 0) {
        type = PipeType.replace(/管线/, "");
    }
    else if (PipeType.indexOf("管点") >= 0) {
        type = PipeType.replace(/管点/, "");
    }
